...
BINVOX_RENDER = True # Turn flag TRUE to render everthing
...
for line in obj_par_file:
    lst = [etl.strip() for etl in line.split(' ')]
    if(len(lst) == 6):
        inner_list = [float(etl.strip()) for etl in lst]
...
def set_K(cam):
    K = [559.551189662, 240.0, 180.0]
    f_pix3d = (K[0] * 32) / (K[1] * 2)
    Image_width = K[1] * 2
    Image_heigth = K[2] * 2
    scene = bpy.context.scene
    scene.render.resolution_x = Image_width
    scene.render.resolution_y = Image_heigth
    cam.data.lens = f_pix3d
